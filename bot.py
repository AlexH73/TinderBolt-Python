...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        await send_text(update, context, "Увас есть хоби?")
    elif dialog.count == 3:
...
async def opener(update, context):
    ...
    await send_text(update, context, "Имя звезды?")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, "ChatGPT 🧠 занимается генерацией. Подождите пару секунд...")
